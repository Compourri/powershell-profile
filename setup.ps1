...
if (!(Test-Path -Path $PROFILE -PathType Leaf)) {
    try {
        ...
        Invoke-RestMethod https://github.com/Compourri/powershell-profile/raw/main/Microsoft.PowerShell_profile.ps1 -OutFile $PROFILE
...
function Install-OhMyPoshTheme {
    param (
        [string]$ThemeName = "pwsh10k",
        [string]$ThemeUrl = "https://raw.githubusercontent.com/Compourri/pwsh10k/refs/heads/master/pwsh10k.omp.json"
    )
    $profilePath = Get-ProfileDir
    if (!(Test-Path -Path $profilePath)) {
...
$themeInstalled = Install-OhMyPoshTheme -ThemeName "pwsh10k"
